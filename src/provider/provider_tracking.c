...
static inline umf_ipc_data_t *
getIpcDataFromIpcHandle(const void *providerIpcData) {
...
static umf_result_t trackingGetIpcHandle(void *provider, const void *ptr,
    ...
    do {
        ...
        if (value) { //cache hit
            ...
            cache_value->handle_id = util_atomic_increment(&IPC_HANDLE_ID);
...
static umf_result_t trackingOpenIpcHandle(void *provider, void *providerIpcData,
    ...
    // Compiler may add paddings to the ipc_mmaped_handle_key_t structure
    // so we need to zero it out to avoid false cache miss.
    ipc_mmaped_handle_key_t key = {0};
    key.remote_base_ptr = ipcUmfData->base;
    key.local_provider = p->hUpstream;
    key.remote_pid = ipcUmfData->pid;

    ipc_handle_cache_entry_t *cache_entry = NULL;
    ret = umfIpcHandleMmapedCacheGet(IPC_MMAPED_CACHE, &key,
    ...
    util_atomic_load_acquire(&(cache_entry->mmaped_base_ptr), &mmaped_ptr);
    if (mmaped_ptr == NULL) {
        util_mutex_lock(&(cache_entry->mmap_lock));
        util_atomic_load_acquire(&(cache_entry->mmaped_base_ptr), &mmaped_ptr);
        if (mmaped_ptr == NULL) {
            ...
            util_atomic_store_release(&(cache_entry->mmaped_base_ptr),
                                      mmaped_ptr);
        }
        util_mutex_unlock(&(cache_entry->mmap_lock));
    }
